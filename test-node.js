// const { _encode } = require("./build-wasm/dist/brotli")
// console.log("_encode", _encode)

const fs = require('fs');

const wasmBuffer = fs.readFileSync('./build-wasm/dist/brotli.wasm');
WebAssembly.instantiate(wasmBuffer, {wasi_snapshot_preview1: {proc_exit: (status) => {throw new Error(`Brotli failed with status ${status}.`)}}}).then(wasmModule => {
    ...
    const encodedSize = encode(0, 22, 0, uint8Array.length, dataPtr, maxOutSize, outputPtr);
    console.log("encodedSize", encodedSize);


    const outBuffer = new Uint8Array(encodedSize);
    outBuffer.set(new Uint8Array(memory.buffer).subarray(outputPtr, outputPtr + encodedSize));
